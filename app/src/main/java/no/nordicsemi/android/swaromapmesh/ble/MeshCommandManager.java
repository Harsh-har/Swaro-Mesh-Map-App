...
// ── Blink config ─────────────────────────────────────────────
    ...
    public static void sendTestCommand(
        ...
        sendMeshCommand(mViewModel, tidCounter, VALUE_ON);
        Log.d(TAG, "Sent CMD=" + HARDCODED_COMMAND + " VALUE=" + VALUE_ON);

        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            sendMeshCommand(mViewModel, tidCounter, VALUE_OFF);
    ...
    public static void startBlink(
        ...
        isBlinking = true;
        Log.d(TAG, "=== BLINK START ===");

        final int[] count = {0}; // blink counter
    ...
    public static void stopBlink(
        ...
        isBlinking = false;
        sendMeshCommand(mViewModel, tidCounter, VALUE_OFF); // force OFF
