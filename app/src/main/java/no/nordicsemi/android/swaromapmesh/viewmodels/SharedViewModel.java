 ...
 // 8 seconds per attempt
    ...
    private void triggerPublicationSetup(@NonNull ProvisionedMeshNode completedNode) {
        ...
        for (String serverKey : provisionedKeys) {
            ...
            // Client Element = Control Node Base + EID
            int clientElementAddr = controlNodeAddr + eid;
            // Feedback Element = Control Node Base + RID (fallback to EID if no RID)
            int feedbackElementAddr = (rid != -1) ? (controlNodeAddr + rid) : clientElementAddr;

            Log.d(TAG, "🚀 Auto-Pub Mapping (SVG Based) for node 0x" + String.format("%04X", serverAddr) + ":");
            Log.d(TAG, "     Client Element: 0x" + String.format("%04X", clientElementAddr) 
                    + " → Server: 0x" + String.format("%04X", serverAddr));
            Log.d(TAG, "     Server: 0x" + String.format("%04X", serverAddr) 
                    + " → Feedback: 0x" + String.format("%04X", feedbackElementAddr));
